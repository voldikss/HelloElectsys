...
function change_name() {
    ...
    else if (name.html() == '罗政亚') {
        name.html('蛋蛋')
    }
    else if (name.html() == '陆子良') {
        name.html('陆司机')
    }
    else if (name.html() == '朱抗洪') {
        name.html('抗洪朱')
    }

    let last_line = $('td[colspan=2]').slice(1, 2).parent();
    let line_string = `
        <tr>
        ...
        <tr onmouseover='sbar(this)' onmouseout='cbar(this)'>
            ...
            <td width="97" class='menu'>
                <div style='position:relative;width:110px;'>
                    <a href='https://github.com/clouduan/HelloElectsys' target='_blank'>Hello Electsys</a>
                </div>
            </td>
        </tr>`;
    last_line.after($(share_link + line_string));
}
...
function optimize_elect_warning() {
    ...
    if (inUrl("/edu/messagePage.aspx")) {
        if (document.body.innerText.indexOf('不能进行该轮选课') > -1) {
...
function optimize_elect1() {
    ...
    radiogroup = $("[name=myradiogroup]", document);
    for (radio_index = 0; radio_index < radiogroup.length; radio_index++) {
        $(radiogroup[radio_index]).click(function () {
            ...
            lcode = $(this).val();
            save_info("lname", lname);
            save_info("lcode", lcode);

            setTimeout(function () {
                $("input[value=课程安排]").trigger("click");
            }, 300);
        });
    }
...
//课程安排页面
function optimize_elect2() {
    if (!inUrl("/edu/lesson/viewLessonArrange.aspx")) {
        return 0;
    }

    $(".button").parent().first().before('<input type="button" id="one-key-elect" style="width: 100px;background-color: #ffc760" value="一键选课">');
    $("#one-key-elect").before('<input type="button" id="monitor" style="width: 100px;background-color: #ffc760" value="监控">');

    let radiogroup = $("[name=myradiogroup]", document).get();
    radiogroup[0].click();   //先默认把第一个老师选中

    $("#one-key-elect").click(function (event) {
        event.stopPropagation();
        monitor_enabled = 1;
        elect_mode = "one-key-elect";
        elect_lesson(elect_mode);
        return 0;
    });

    $("#monitor").click(function (event) {
        event.stopPropagation();
        save_info("hide_info", 0);
        monitor_enabled = 1;
        elect_mode = "monitor";
        setInterval("elect_lesson('monitor')", monitor_interval);
    })
}

//选定此课程
function elect_lesson(elect_mode) {
    ...
    form = $("form", document);
    url = base_url + "/edu/lesson/" + form.attr("action");
    //POST
    $.ajax({
        type: 'POST',
        url: url,
        ...
        success: function (data) {
            ...
            if (data.indexOf("人数已满") > -1 || data.indexOf("请选择课程")>-1) {
                ...
                if (elect_mode == "monitor") {
                    save_info("monitor_status", "OFF(刷新频率过高)");
                    return 0;
                }
                else {
                    alert("请勿频繁刷新页面！");
            ...
            goto_main_page(elect_mode);
        },
        error: function () {
            save_info("monitor_status", "OFF(请检查网络连接)");
...
//必须回到主页然后才能提交成功
function goto_main_page(elect_mode) {
    ...
    $.get(url, function (document) {
        ...
        submit(url, data, elect_mode);
        return 0;
    })
}

//选课提交
function submit(url, data, elect_mode) {
    $.ajax({
        type: 'POST',
        url: url,
        data: data,
...
//保存所监控的课程信息
function save_info(key, item) {
    switch (key) {
        case "lname":
            chrome.storage.local.set({lname: item}, result => {
            });
            break;
        case "lcode":
            chrome.storage.local.set({lcode: item}, result => {
            });
            break;
        case "tname":
            chrome.storage.local.set({tname: item}, result => {
            });
            break;
        case "lstatus":
            chrome.storage.local.set({lstatus: item}, result => {
            });
            break;
        case "monitor_status":
            chrome.storage.local.set({monitor_status: item}, result => {
            });
            break;
        case "hide_info":
            chrome.storage.local.set({hide_info: item}, result => {
            });
            break;
    }
    chrome.storage.local.set({times: times}, result => {
    });
}

//一键退课
function one_key_remove() {
    if (!inUrl("edu/student/elect/removeLessonFast.aspx?second=y")) {
        return 0;
    }
...
function processLidQueue() {
    ...
    sub_button = $('[value=课程安排]', document);
...
//		console.log(lid);
    ...
    lessontr.click(function () {
        ...
        if ($(this).attr("clicked") == "1") {
            $(this).attr("clicked", "0");
            clearDraw_lid($(this).attr("lid"));
        } else {
            clearDraw_lid($(this).attr("lid"));
            draw_lesson($(this).attr("lid"), 1);
            $(this).attr("clicked", "1");
        }
